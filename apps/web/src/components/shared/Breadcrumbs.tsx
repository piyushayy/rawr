...
export const Breadcrumbs = ({ items }: BreadcrumbsProps) => {
    ...
    const jsonLd = {
        ...
        "itemListElement": items.map((item, index) => ({
            ...
            "position": index + 1,
            "name": item.label,
            "item": process.env.NEXT_PUBLIC_SITE_URL ? `${process.env.NEXT_PUBLIC_SITE_URL}${item.href}` : item.href
        }))
    };

    return (
        <nav aria-label="Breadcrumb" className="py-4 text-xs font-bold uppercase tracking-widest text-gray-500">
            <script

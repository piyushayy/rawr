          ...
          <div className="flex items-center gap-2 mt-2">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  key={star}
                  className={`w-5 h-5 ${star <= Math.round(average) ? "fill-rawr-black text-rawr-black" : "text-gray-300"}`}
                />
              ))}
            </div>
        ...
        <div className="space-y-6">
          ...
          {reviews?.map((review: any) => (
            <div key={review.id} className="border-b-2 border-gray-200 pb-6">
                ...
                <div className="flex items-center gap-2 mb-2">
                  <div className="flex">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <Star
                        key={star}
                        className={`w-4 h-4 ${star <= review.rating ? "fill-rawr-black text-rawr-black" : "text-gray-300"}`}
                      />
                    ))}
                  </div>
                  {review.is_verified && (
              ...
              <p className="font-bold text-gray-800 leading-relaxed mb-3">
                {review.comment}
              </p>
                ...
                <div className="flex gap-2 mt-3 overflow-x-auto pb-2 snap-x">
                  {review.images.map((img: string, idx: number) => (
                    ...
                    <img
                      ...
                      src={img}
                      alt="Review"
                      className="w-24 h-24 object-cover border border-rawr-black shrink-0 snap-start"
                    />
                  ))}
                </div>
              )}

                            ...
                            <div>
                                ...
                                <div className="pt-4">
                                    {!hasDropped && product.release_date ? (
                                        ...
                                        <div ref={(el) => { if (el) setMainButtonInView(true); }} className="w-full space-y-4">
                                            {(isVariantSoldOut || product.soldOut) ? (
                                                <div className="border-2 border-rawr-black p-4 bg-gray-50 space-y-4">
                                                    <p className="font-bold uppercase text-sm tracking-widest text-gray-500 text-center">Get Notified on Restock</p>
                                                    <form onSubmit={async (e) => {
                                                        ...
                                                        const { subscribeToStock } = await import("./actions");
                                                        const res = await subscribeToStock(product.id, email);
                                                        if (res?.error) toast.error(res.error);
                                                        else {
                                                        ...
                                                        <input type="email" name="email" required placeholder="Enter your email..." className="w-full p-3 border-2 border-rawr-black outline-none font-body" />
                                                        <Button type="submit" className="w-full bg-rawr-black text-white hover:bg-gray-800 h-12 uppercase tracking-widest">
                                                            Notify Me
                                                        </Button>
                                                    </form>
                                                </div>
                                            ...
                                            <InView onChange={setMainButtonInView} className="absolute inset-0 pointer-events-none" />
                                        </div>
                                    )}
                                    <div className="mt-6">
                        ...
                        >
                            <div className="flex gap-4 items-center">
                                <div className="flex-1">
                                    <h4 className="font-heading font-bold uppercase text-sm truncate">{product.title}</h4>
                                    <p className="font-mono text-xs text-rawr-red font-bold"><Price amount={selectedVariant?.price_override || product.price} /></p>
                                </div>
                                <Button
                                    size="lg"

...
export const FeedPost = ({ post, currentUserId }: FeedPostProps) => {
    ...
    return (
        <article className="border-2 border-rawr-black bg-white mb-8 group">
            ...
            <div className="p-4 flex items-center gap-3 border-b-2 border-rawr-black bg-gray-50">
                <div className="w-10 h-10 rounded-full bg-rawr-black overflow-hidden border border-gray-300 relative">
                    {post.user?.avatar_url ? (
                        <Image src={post.user.avatar_url} alt="User" fill className="object-cover" />
                    ) : (
            ...
            {/* Image */}
            <div className="relative aspect-square md:aspect-[4/5] bg-gray-200 overflow-hidden" onDoubleClick={handleLike}>
                <Image
                    src={post.image_url}
                    alt="Community Post"
                    fill
                ...
                {/* Heart Pop Animation */}
                {isAnimating && (
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none" onAnimationEnd={() => setIsAnimating(false)}>
            ...
            <div className="p-4 border-t-2 border-rawr-black space-y-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={handleLike} className="hover:text-rawr-red transition-colors flex items-center gap-2 group/like">
                            <Heart className={`w-6 h-6 transition-all ${isLiked ? 'fill-rawr-red text-rawr-red' : 'text-rawr-black group-hover/like:text-rawr-red'}`} />
                ...
                {post.caption && (
                    <div className="text-sm md:text-base font-medium">
                        <span className="font-bold uppercase mr-2">{post.user?.full_name || 'User'}:</span>
                        {post.caption}
                    </div>
                )}
            </div>

...
export default function CheckoutPage() {
    ...
    const recoverCart = async () => {
      ...
      if (recoveryId && items.length === 0) {
        ...
        const { data: order } = await supabase
          .from("orders")
          .select("order_items(*, products(title, images, size))")
          .eq("id", recoveryId)
          .single();
  ...
  const total = items.reduce(
    (acc, item) => item.price * (item.quantity || 1) + acc,
    0,
  );
  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-rawr-white flex flex-col items-center justify-center p-4">
        <h1 className="text-4xl font-heading font-black uppercase mb-4">
          Your stash is empty
        </h1>
        <Link href="/shop">
    ...
    <div className="min-h-screen bg-rawr-white lg:grid lg:grid-cols-2">
      ...
      <div className="p-8 lg:p-12 bg-gray-100 border-r-2 border-rawr-black block">
        <div className="max-w-xl mx-auto">
          <h2 className="text-2xl font-heading font-black uppercase mb-8">
            Order Summary
          </h2>
          ...
          <div className="border-t-2 border-rawr-black mt-8 pt-8 space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <span
                className={
                  shippingCost === "FREE" ? "text-green-600 font-bold" : ""
                }
              >
                {shippingCost === "FREE" ? (
                  "FREE"
                ) : (
                  <Price amount={Number(shippingCost)} />
                )}
              </span>
            </div>
          ...
          {checkoutStep === "details" ? (
            <div className="space-y-6 py-12 text-center border-2 border-rawr-black p-8 bg-gray-50">
              ...
              <h3 className="text-2xl font-heading font-black uppercase mb-2">
                Join The Cult to Checkout
              </h3>
              <p className="font-body text-gray-500 mb-8 max-w-sm mx-auto">
                We need your details to secure your stash, provide accurate
                shipping, and prevent bots from eating our inventory.
              </p>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                {!isAuthenticated && (
                  <button
                    ...
                    className="text-sm underline hover:text-rawr-red"
                  >
                    Edit Details
                  </button>
                )}
              ...
              {paymentMethod === "razorpay" && (
                <div className="text-center py-8">
                  <p className="mb-6 font-bold text-xl">
                    Order Total: <Price amount={finalTotal} />
                  </p>

...
export async function createOrder(formData: FormData) {
  ...
  if (result.email && result.orderId && result.total) {
    // Check required props
    sendOrderConfirmation(result.email, result.orderId, result.total).catch(
      console.error,
    );
  } else if (user.email && result.orderId && result.total) {
    // Fallback to user auth email
    sendOrderConfirmation(user.email, result.orderId, result.total).catch(
      console.error,
    );
  }

...
export default async function CustomerListPage({ searchParams }: { searchParams: Promise<{ q?: string, sort?: string }> }) {
    ...
    if (error) {
        ...
        return <div className="p-8 text-red-500">Error loading customers. Please run `supabase/crm_v2.sql`.</div>;
    }

    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-3xl font-heading font-black uppercase">Customers (CRM)</h2>
            ...
            <div className="bg-white p-4 border border-gray-200 flex flex-col md:flex-row gap-4 justify-between">
                ...
                <div className="flex gap-2 items-center">
                    ...
                    <div className="flex gap-2">
                        <Link href={`/admin/customers?sort=joined_desc&q=${query}`}><Button size="sm" variant={sort === 'joined_desc' ? 'default' : 'outline'} className="text-xs h-8">Newest</Button></Link>
                        <Link href={`/admin/customers?sort=spend_desc&q=${query}`}><Button size="sm" variant={sort === 'spend_desc' ? 'default' : 'outline'} className="text-xs h-8">Top Spenders</Button></Link>
                        <Link href={`/admin/customers?sort=clout_desc&q=${query}`}><Button size="sm" variant={sort === 'clout_desc' ? 'default' : 'outline'} className="text-xs h-8">Most Clout</Button></Link>
                    </div>
                </div>
            </div>

            <div className="bg-white border text-sm border-gray-200 overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                            <th className="p-4 font-bold uppercase text-gray-500">Customer</th>
                            <th className="p-4 font-bold uppercase text-gray-500">Clout Score</th>
                            <th className="p-4 font-bold uppercase text-gray-500">Orders</th>
                            <th className="p-4 font-bold uppercase text-gray-500">Lifetime Value</th>
                            <th className="p-4 font-bold uppercase text-gray-500">Last Seen</th>
                            <th className="p-4 font-bold uppercase text-gray-500 text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                        ...
                        {customers?.map((customer: any) => (
                            <tr key={customer.user_id} className="hover:bg-gray-50 group">
                                <td className="p-4">
                                    <div className="font-bold text-base">{customer.full_name || 'Anonymous'}</div>
                                ...
                                <td className="p-4 text-right">
                                    <Link href={`/admin/customers/${customer.user_id}`}>
                                        <Button size="sm" variant="ghost" className="hover:bg-black hover:text-white transition-colors"><Eye className="w-4 h-4" /></Button>
                                    </Link>
                                </td>
                            </tr>
                        ))}
            ...
            <p className="text-center text-xs text-gray-400 uppercase">Showing top 50 matches</p>
        </div>
    );
}

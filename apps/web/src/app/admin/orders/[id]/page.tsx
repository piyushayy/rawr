...
export default async function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <div className="space-y-8">
            <Link href="/admin/orders" className="flex items-center text-sm hover:underline text-gray-500">
                <ArrowLeft className="w-4 h-4 mr-2" /> Back to Orders
            </Link>

            <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-4xl font-heading font-black uppercase">Order #{order.id.slice(0, 8)}</h1>
                    <p className="text-gray-500">{new Date(order.created_at).toLocaleString()}</p>
                </div>
                <div className={`px-4 py-2 rounded text-lg font-bold uppercase ${order.status === 'paid' ? 'bg-green-100 text-green-800' :
                    ...
                    <Card>
                        <CardContent className="p-6">
                                    ...
                                    <div key={item.id} className="flex gap-4 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between">
                                                ...
                                                <div className="font-bold"><Price amount={item.price} /></div>
                                            </div>
                            ...
                            <div className="mt-4 pt-4 border-t border-gray-200 text-right">
                                <span className="font-bold text-xl">Total: <Price amount={order.total} /></span>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-gray-50">
                        <CardContent className="p-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <Button variant="secondary" disabled>Print Label (Coming Soon)</Button>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                        ...
                        <CardContent className="p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="mt-4 pt-4 border-t border-gray-100">
                                    <p className="text-xs uppercase font-bold text-gray-400">Clout Score</p>
                                    <p className="font-mono text-lg">{order.profiles?.clout_score}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>

    ...
    return (
        <div className="p-8">
            <div className="flex justify-between items-center mb-8">
                <h1 className="text-4xl font-heading font-black uppercase">The Manifesto (Blog)</h1>
                        ...
                        {articles?.map((article) => (
                            <tr key={article.id} className="hover:bg-gray-50">
                                ...
                                <td className="p-4 font-bold">
                                    {article.title}
                                    <p className="text-xs text-gray-400 font-normal mt-1">/{article.slug}</p>
                                </td>
                                <td className="p-4">
                                    {article.published ? (
                                        <span className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded font-bold uppercase">Published</span>
                                    ) : (
                                        <span className="bg-yellow-100 text-yellow-800 text-xs px-2 py-1 rounded font-bold uppercase">Draft</span>
                                    )}
                                </td>
                        ...
                        {articles?.length === 0 && (
                            <tr>
                                <td colSpan={5} className="p-8 text-center text-gray-500">
                                    No articles written yet. The world is waiting for your manifesto.
                                </td>
                            </tr>
                        )}
                    </tbody>

    ...
    return (
        <div className="space-y-6 font-body text-lg leading-relaxed text-gray-800">
            {blocks.map((block, index) => {
                // Header 1
                if (block.startsWith('# ')) {
                    return <h2 key={index} className="text-4xl font-heading font-black uppercase mt-12 mb-6">{block.replace('# ', '')}</h2>
                }
                // Header 2
                if (block.startsWith('## ')) {
                    return <h3 key={index} className="text-2xl font-heading font-black uppercase mt-8 mb-4">{block.replace('## ', '')}</h3>
                }
                // Header 3
                if (block.startsWith('### ')) {
                    return <h4 key={index} className="text-xl font-bold uppercase mt-6 mb-2">{block.replace('### ', '')}</h4>
                }
                // Blockquote
                if (block.startsWith('> ')) {
                    return (
                        <blockquote key={index} className="border-l-4 border-rawr-red pl-6 py-2 my-8 italic text-xl text-gray-600 bg-gray-50">
                            {block.replace('> ', '')}
                        </blockquote>
                    );
                }
                ...
                if (imgMatch) {
                    return (
                        <div key={index} className="my-12 relative aspect-video w-full border-2 border-rawr-black">
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    const { data: article } = await supabase.from('articles').select('title, excerpt, image_url').eq('slug', slug).single();
    ...
    return (
        <article className="min-h-screen bg-white">
            ...
            <div className="relative h-[60vh] md:h-[80vh] w-full border-b-2 border-rawr-black bg-rawr-black">
                {article.image_url && (
                    <>
                        <Image src={article.image_url} alt={article.title} fill className="object-cover opacity-80" priority />
                ...
                <div className="absolute inset-x-0 bottom-0 p-8 md:p-16 z-20">
                    <div className="container mx-auto max-w-4xl">
                        <Link href="/manifesto" className="inline-flex items-center text-white mb-8 hover:text-rawr-red transition-colors font-bold uppercase tracking-widest text-sm">
            ...
            <div className="container mx-auto max-w-3xl px-4 py-16 md:py-24">
                ...
                <div className="mt-24 pt-12 border-t-2 border-rawr-black">
                    <div className="bg-gray-50 p-8 border border-gray-200 text-center">
                        <h3 className="font-heading font-bold uppercase text-2xl mb-2">Join the Conversation</h3>
                        <p className="text-gray-600 mb-6">Drop into the gallery and show us how you wear the raw truth.</p>
                        <Link href="/shop" className="inline-block bg-rawr-black text-white px-8 py-4 font-bold uppercase tracking-widest hover:bg-rawr-red transition-colors shadow-[4px_4px_0px_0px_#050505]">
                            Shop The Look
                        </Link>
                    </div>
                </div>
